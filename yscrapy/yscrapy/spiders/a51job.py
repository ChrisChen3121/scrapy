...
function main(splash, args)
  ...
  return pages
    ...
    def __init__(self, city='shanghai'):
        # host = 'www.51job.com'
        self._url_root = 'http://www.51job.com/{}'.format(city)
    ...
    def parse_category(self, response):
        for node in response.xpath('//div[@class="hle"]/div/a/@href'):
    ...
    def parse_company_page(self, response):
        ...
        if company_info:
            # yield company_info # TODO: may dump company
            yield SplashRequest(
                url=response.url,
                # headers=self._headers,
    ...
    def _parse_company_info(self, response):
        ...
        company_info['address'] = response.xpath(
            '//div[@class="tBorderTop_box bmsg"]//p/text()').extract_first(
            ).strip()
        return company_info
